...
export const PotRecipeUnlockSection = ({capacity, cumulativeCost, potInfo, meals, unlockedRecipes}: Props) => {
  ...
  return (
    <Flex direction="col" className="button-bg gap-1.5 rounded-lg p-2">
      <Flex direction="row" className="gap-3" center>
        <Flex direction="row" className="gap-1" center noFullWidth>
        ...
        <Flex direction="row" className="gap-1" center noFullWidth>
          ...
          <div>
            {!capacity || capacity < potInfo.capacity ?
              `${formatInt(cumulativeCost)} (+${formatInt(potInfo.cost)})` :
              '-'}
          </div>
        </Flex>
        <Flex direction="row" className="gap-1" center noFullWidth>
